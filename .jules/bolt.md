...
## 2025-06-17 - Viewport Object Reallocation in Framer Motion
**Learning:** Defining inline objects for props like `viewport` (e.g., `viewport={{ once: true, amount: 0.2 }}`) on `<motion.div>` components triggers object reallocation and potential VDOM diffing overhead on every render, even outside of `.map()` loops. For highly nested components or components with many scroll-triggered animations (like `LearnMore.tsx`), this causes unnecessary garbage collection and negatively impacts performance.
**Action:** Always extract static `viewport` configuration objects to module-level constants (e.g., `const viewportConfigOnce = { once: true, amount: 0.2 } as const;`) and reference them in `motion` elements to ensure referential equality.
