...
## 2025-06-15 - Extracting Inline Viewport Objects for Motion Components
**Learning:** Defining inline object literals for Framer Motion's `viewport` prop (e.g., `viewport={{ once: true, amount: 0.2 }}`) directly on `motion.div` components forces React to create a new object reference on every render. While Framer Motion internally treats these as read-only, passing an unstable object reference down the tree creates unnecessary garbage collection churn and breaks shallow equality comparisons for any pure components down the line. It's a small but highly pervasive anti-pattern in large animation-heavy codebases.
**Action:** Always extract inline static `viewport` configuration objects into module-level constants (e.g., `const VIEWPORT_ONCE = { once: true } as const;`) to ensure referential stability and eliminate unnecessary memory reallocation on each render cycle.
