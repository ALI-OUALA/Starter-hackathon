...
## 2025-06-17 - Extracting Inline Viewport Props
**Learning:** Defining inline `viewport={{ once: true }}` configuration objects directly on `<motion.div>` components causes React to allocate a new object on every render. Because Framer Motion components are heavily used for scroll animations throughout the layout, this adds unnecessary garbage collection overhead and forces unnecessary vDOM diffing.
**Action:** Always extract static `viewport` configuration objects into module-scoped constants (e.g., `const viewportConfig = { once: true, amount: 0.2 } as const;`) to ensure referential equality across renders.
