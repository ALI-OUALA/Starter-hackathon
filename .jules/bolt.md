...
## 2025-07-15 - Extracting Inline Viewport Configuration Objects
**Learning:** Defining inline `viewport={{ once: true, amount: 0.2 }}` configurations for Framer Motion `<motion.div>` elements causes React to reallocate object references on every render. Because these objects configure the Intersection Observer thresholds internally in Framer Motion, passing a new object reference on every render can cause Framer Motion to unnecessarily tear down and recreate IntersectionObserver instances or trigger internal re-renders.
**Action:** Extract inline `viewport` configuration objects into module-level constants (e.g., `viewportOnceAmount`, `viewportOnce`) to ensure stable prop references and prevent unnecessary observer recreation.
