...
## 2025-06-17 - Extracting Inline Viewport Configuration Objects
**Learning:** In React components utilizing Framer Motion, defining inline objects like `viewport={{ once: true, amount: 0.2 }}` directly on `<motion.div>` elements causes React to create a new object reference on every render. Even though these variants are static, this increases GC pressure and breaks pure component shallow comparisons. When grouped across the codebase, this amplifies the problem.
**Action:** Always extract static `viewport` configuration objects into module-level constants (e.g., `const viewportConfig = { once: true } as const;`) outside the component to ensure referential equality across renders, avoiding garbage collection churn and unnecessary vDOM diffing overhead.
