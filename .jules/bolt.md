...
## 2025-06-17 - React Component Memoization Without Props
**Learning:** Wrapping static, prop-less layout sub-components (such as `ScheduleTimeline` extracted from `EventCalendar`) in `React.memo()` successfully prevents unnecessary React VDOM diffing and re-rendering when the parent state changes (such as the main `currentView` in `App.tsx` or any internal state that might be added to the parent later).
**Action:** Always identify and wrap large, static, prop-less sub-components defined within page-level files with `React.memo()` to isolate render cycles.
