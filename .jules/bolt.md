...
## 2025-06-17 - Extracting Inline Viewport Configs
**Learning:** Defining inline object literals for the `viewport` prop (e.g., `viewport={{ once: true, amount: 0.2 }}`) directly on `<motion.div>` components causes React to allocate new objects on every render cycle. While this overhead is small individually, doing this consistently across many animated elements, especially in frequently updated components, increases garbage collection churn and negates React's shallow equality optimizations.
**Action:** Always extract static `viewport` configurations into module-scoped constants like `viewportConfig` or `viewportOnceConfig`, taking care to maintain `as const` for referential equality and to prevent unnecessary allocations.
