...
## 2025-06-14 - Incomplete Extracting of Motion Props
**Learning:** Extracting only `initial` and `animate`/`whileInView` properties from a `<motion.div>` to module-level constants reduces some garbage collection churn but fails to achieve full referential equality if other object literal props (like `transition={{ ... }}`, `style={{ ... }}`, or `viewport={{ ... }}`) are left inline on the same element. This means React will still recreate object references and break `React.memo()` down the tree, defeating the primary purpose of the optimization.
**Action:** When extracting framer-motion props to achieve referential equality, always extract *all* object literal props (including `transition`, `style`, and `viewport`) attached to that specific `motion` element, not just the variant states.
