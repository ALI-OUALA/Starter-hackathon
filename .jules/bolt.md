...
## 2025-06-15 - Extracting Inline Viewport Props for Framer Motion
**Learning:** Passing inline object configurations to props like `viewport={{ once: true, amount: 0.2 }}` on Framer Motion's `<motion.div>` components triggers object reallocation on every render cycle. While often overlooked because they aren't explicit `variants`, these inline configurations still add to garbage collection overhead and can break React's pure component shallow comparisons.
**Action:** Always extract inline configurations for structural props like `viewport` to module-level constants (e.g., `const viewportOnce = { once: true } as const;`) to maintain referential equality and reduce memory churn.
