...
## 2024-06-17 - Extracting Inline Viewport Objects in Framer Motion
**Learning:** Defining inline object literals for the `viewport` prop (like `viewport={{ once: true, amount: 0.2 }}`) on `motion` elements within React components causes the object reference to be recreated on every render. Because the reference changes, Framer Motion may needlessly tear down and re-create its `IntersectionObserver` instances under the hood, adding layout thrashing and GC overhead.
**Action:** Always extract static `viewport` configuration objects into module-scoped constants (e.g., `viewportOnceAmount = { once: true, amount: 0.2 } as const`) to maintain reference stability and prevent unnecessary observer re-evaluations on render. Ensure these module-level constants are placed *after* any `import` statements at the top of the file to comply with ES module syntax and standard linting rules.
